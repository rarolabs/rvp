...
public class TornarAutoridadeAsyncTask extends AsyncTask<Object, Void, Boolean> {
    private static BackendServices backendServices = null;
    private final Activity activity;
    private Context context;
    private SharedPreferences settings;

    public TornarAutoridadeAsyncTask(Activity activity) {
    ...
    protected Boolean doInBackground(Object... params) {
        ...
        try {
            ...
            if(checked) {
                backendServices.tornarAutoridade(membroId);
            }else{
                backendServices.retirarPermissaoAutoridade(membroId);
            }
            ...
            activity.runOnUiThread(new Runnable() {
                public void run() {
                    ((PerfilActivity)activity).errorToggleAutoridade(e.getDescricao());
                }
            });
            return false;
        }
    ...
    protected void onPostExecute(final Boolean result) {
        if(result) {
            activity.runOnUiThread(new Runnable() {
                public void run() {
                    ((PerfilActivity) activity).okToggleAutoridade();
                }
            });
        }
    }
